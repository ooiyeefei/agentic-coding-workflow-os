...
from spanweave.evidence import CommandOutput, EvidencePack, Finding, Severity, Verdict
from spanweave.security import redact
...
def run_uat(
    ...
    *,
    ...
    env["SPANWEAVE_UAT_APP_PATH"] = str(Path(request.app_path).resolve())
    if test_plan:
        env["SPANWEAVE_UAT_TEST_PLAN"] = test_plan
    if request.notes:
        env["SPANWEAVE_UAT_NOTES"] = request.notes
...
def resolve_skill_path(skill_path: str | None = None) -> Path:
    candidate = (
        skill_path
        or os.getenv("SPANWEAVE_UAT_SKILL_PATH")
    ...
    if not path.exists():
        raise UATExecutionError(
            ...
            f"{path}. Set SPANWEAVE_UAT_SKILL_PATH to a valid executable or skill directory."
        )
    return path
