...
DAEMON_SECRET_HEADER = "X-Spanweave-Secret"
...
def _secret_path(repo_root: Path) -> Path:
    return repo_root / ".spanweave" / "daemon" / "secret.txt"
...
async def require_shared_secret(
    request: Request,
    x_spanweave_secret: str | None = Header(default=None, alias=DAEMON_SECRET_HEADER),
) -> None:
    config: DaemonConfig = request.app.state.daemon_config
    if x_spanweave_secret is None or not compare_digest(x_spanweave_secret, config.shared_secret):
        raise HTTPException(
