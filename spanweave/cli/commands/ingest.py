...
from spanweave.session import ingest_transcript
...
@click.command(
    ...
    epilog=build_help_epilog(
        notes=("Extracted records are written to .spanweave/memory/ as typed markdown records.",),
        examples=(
            "spanweave ingest --from transcript.md --tool generic",
            "spanweave ingest --from rollout.jsonl --tool codex --json",
        ),
    ),
)
@click.option(
...
def ingest_command(source_path: Path, source_tool: str, repo: Path, json_output: bool) -> None:
    """Ingest a transcript into durable Spanweave memory."""
