...
from spanweave.cli.formatters import build_help_epilog, echo_json, render_daemon_status
...
def _state_path(repo_path: Path) -> Path:
    return repo_path / ".spanweave" / "daemon" / "state.json"
...
@click.group(
    ...
    epilog=build_help_epilog(
        ...
        examples=(
            ...
            "spanweave daemon status --repo . --json",
...
@daemon_group.command(
    ...
    epilog=build_help_epilog(
        ...
        examples=(
            "spanweave daemon start --repo .",
            "spanweave daemon start --repo . --json",
        ),
    ),
)
@click.option(
...
@daemon_group.command(
    ...
    epilog=build_help_epilog(
        ...
        examples=(
            "spanweave daemon stop --repo .",
            "spanweave daemon stop --repo . --json",
        ),
    ),
)
@click.option(
...
@daemon_group.command(
    ...
    epilog=build_help_epilog(
        notes=(
            "Status is read from .spanweave/daemon/state.json until the real daemon lands in W16.",
        ),
        examples=(
