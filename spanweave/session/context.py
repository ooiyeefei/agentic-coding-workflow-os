...
from spanweave.compiler import SampleDoc, Source, compile_packet
...
def generate_context(target: str, token_limit: int = 4000) -> str:
    ...
    objective = (
        f"Standalone Spanweave memory context for {target.strip() or 'external tool'}. "
...
def _context_sources(records: list[MemoryRecord]) -> list[Source]:
    return [
        SampleDoc(
            ...
            path=f".spanweave/memory/{record.path_fragment.as_posix()}",
        )
