...
- **Prompt pattern library** (`.atelier/defaults/prompt_patterns/`) — defensive prompt-craft as composable, version-controlled markdown files. Each pattern carries YAML frontmatter declaring its scope (which personas can `@import` it). Initial seeded patterns:
  ...
  Persona system prompts opt in via a `patterns:` array in their frontmatter; the loader concatenates pattern bodies into the rendered system prompt. Reusable, shareable, contributable upstream — extends Spanweave's "files-first, format-over-platform" thesis from typed records to prompt discipline.
