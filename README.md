...
Spanweave solves this with **files in git** as the shared substrate. Not a platform. Not another tool to adopt. A `.spanweave/` directory in your repo that every agent tool can read, containing decisions, evidence, context, and rules in markdown.

**Session swap**: `spanweave resume --agent claude-code` — picks up where Codex left off, full context.
**Multi-agent**: `spanweave prompt --role coder --agent codex` + `spanweave prompt --role reviewer --agent claude-code` — each gets the right context for their role.
**Team collaboration**: Alice uses Claude Code, Bob uses Codex. Both read/write `.spanweave/memory/`. Git syncs. No shared platform needed.
...
- `spanweave/` — Python control plane
- `.spanweave/defaults/` — shipped opinionated defaults (personas, skills, rules, workflows, policy)
