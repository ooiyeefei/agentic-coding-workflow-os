...
- **Fields**:
  ...
  - `path`: `.spanweave/runs/<run_id>/`
  - `lock_path`: `.spanweave/runs/<run_id>/.lock`
- **Validation rules**:
  - `run_id` must satisfy the project's existing run ID validation rules.
  - `path` must remain under `.spanweave/runs/`.
...
- **Fields**:
  ...
  - `path`: `.spanweave/runs/<run_id>/stages/<stage_id>/`
- **Validation rules**:
...
- **Fields**:
  - `path`: `.spanweave/runs/<run_id>/.lock`
