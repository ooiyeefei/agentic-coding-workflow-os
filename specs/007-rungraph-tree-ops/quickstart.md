...
2. Run `uv run ruff check atelier/rungraph tests/test_rungraph.py`.
3. Run `uv run pyright atelier/rungraph`.

## Manual spot checks

1. Create a run and confirm `.atelier/runs/<run_id>/run.md`, `audit.jsonl`, `stages/`, and `.lock` exist.
