...
**Created**: 2026-04-22  
**Status**: Implemented  
**Input**: User description: "integration tests validating full Atelier workflow end-to-end + CI pipeline"

## Clarifications

### Session 2026-04-22

- Q: Which LLM mode should CI use for workflow validation? → A: CI uses deterministic mock LLM behavior by default, with real LLM calls enabled only when `ATELIER_INTEGRATION_REAL_LLM=1`.
...
- Q: How should repeatability be enforced? → A: Every integration test must create isolated repo-local state and must not depend on cached runs, prior `.atelier/` contents, or previously generated artifacts.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Validate The Full Workflow Run (Priority: P1)

As a maintainer, I can run one deterministic end-to-end integration test against the demo app and demo issue, so Phase 0 has a single proof point that the default Atelier workflow still works from start to finish.
...
3. **Given** a maintainer with real model credentials, **When** `ATELIER_INTEGRATION_REAL_LLM=1` is set, **Then** the integration fixtures can opt into real persona calls without changing test code or CI defaults.

### Edge Cases

- The integration suite must not reuse `.atelier/runs/` data from earlier tests or earlier local runs.
...
- **FR-005**: `tests/integration/conftest.py` MUST provide shared isolated fixtures for repo-root setup, deterministic mock LLM behavior, and opt-in real LLM execution controlled by `ATELIER_INTEGRATION_REAL_LLM`.
...
- **FR-007**: The integration harness MUST be repeatable: every test run starts from a clean temporary workspace and does not depend on pre-existing `.atelier/` state.
...
- **SC-003**: Running `uv run pytest tests -q` passes without relying on leftover `.atelier/` state from previous test runs.
...
- **SC-006**: Setting `ATELIER_INTEGRATION_REAL_LLM=1` enables real-model integration coverage while leaving the default CI path unchanged.
