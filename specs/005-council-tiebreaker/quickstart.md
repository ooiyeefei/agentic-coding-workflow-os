...
2. Run `uv run ruff check spanweave/council spanweave/memory tests/test_council.py`.
3. If council-specific memory helpers were added, run `uv run pyright spanweave/council spanweave/memory`.
...
4. Inspect the generated `.spanweave/memory/council_reports/*.md` file and confirm it includes the selected models, full vote breakdown, and final verdict.
