...
2. Run `uv run ruff check atelier/policy tests/test_policy.py`.
...
4. Create a temporary `.atelier/runs/<run_id>/audit.jsonl` containing a cost total of `4.99`, then confirm `check_cost(run_id, 0.01)` returns `True`.
