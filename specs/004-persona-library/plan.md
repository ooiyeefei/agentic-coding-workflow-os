...
**Structure Decision**: Keep persona logic isolated inside `atelier/personas/`, store shipped prompt definitions in `.atelier/defaults/personas/`, and make only the minimal adjacent updates in `atelier/llm/` and `.atelier/defaults/models/` that are necessary for genuine capability-gated routing on default manifests.
