...
3. Run `uv run ruff check atelier/personas tests/test_personas.py tests/test_llm_adapter.py`.
4. Run `uv run pyright atelier/personas`.
...
3. Read `.atelier/defaults/personas/reviewer.md` and confirm the prompt mentions `execute`, `paste actual output`, and `incomplete without execution evidence`.
