...
## Decision 2: Treat `spanweave run` as "create and surface the stored run"

- **Decision**: `spanweave run --issue <ref>` creates a run and returns the stored run snapshot immediately; `spanweave run resume <run_id>` only advances state where the persisted workflow already allows a safe approval transition.
...
- **Decision**: Repo-scoped commands accept `--repo PATH` where the command surface makes sense and resolve `.spanweave/` relative to that repository.
- **Rationale**: The acceptance example already uses `spanweave run --issue 42 --repo .`, and the current codebase stores state in repo-relative paths. Explicit repo targeting works cleanly in scripts.
- **Alternatives considered**:
...
- **Decision**: `spanweave init` creates missing `.spanweave/` directories without overwriting existing user state, while `spanweave cleanup` requires either an interactive confirmation prompt or `--yes`.
...
- **Alternatives considered**:
  - Make `init` fail when `.spanweave/` already exists: rejected because it turns a safe bootstrap command into a nuisance.
...
- **Decision**: `spanweave daemon start|stop|status` manages repository-scoped placeholder state instead of inventing a real HTTP listener or PID-based supervisor.
...
## Decision 7: Implement `grep` in Python over `.spanweave/` text artifacts

- **Decision**: `spanweave grep <pattern>` searches repo-local `.spanweave/` text artifacts in Python instead of shelling out to an external grep tool.
