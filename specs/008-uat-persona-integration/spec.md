...
- Q: How is the external UAT skill located? → A: The runner resolves `ATELIER_UAT_SKILL_PATH`, defaulting to `/home/fei/fei/code/hackathon/ccc/skills/uat-testing`, and callers may override it for local worktrees or tests.
...
As a workflow stage, I can invoke the user's existing UAT skill against a concrete app path with test credentials so Atelier verifies a real user-facing flow instead of only unit or integration tests.
...
**Why this priority**: UAT still has to behave like an Atelier persona, not a one-off script. Capability routing and normalized responses keep the workflow architecture coherent.
...
- What happens when `ATELIER_UAT_SKILL_PATH` points to a missing path, a non-executable file, or a directory without a resolvable entrypoint?
...
- **FR-001**: The system MUST provide a `UAT` persona in `atelier/personas/uat.py` that extends the shared `Persona` base.
- **FR-002**: The UAT persona MUST load its system prompt from `.atelier/defaults/personas/uat.md`.
...
- **FR-005**: The system MUST provide a subprocess wrapper in `atelier/personas/uat_runner.py` that resolves the external UAT skill path from `ATELIER_UAT_SKILL_PATH`, defaulting to `/home/fei/fei/code/hackathon/ccc/skills/uat-testing`.
...
- **SC-004**: Invoking the runner with an invalid `ATELIER_UAT_SKILL_PATH` raises a clear error instead of producing a silent pass.
...
- The external UAT skill can consume target-app context and credentials from subprocess inputs without requiring Atelier to reimplement the browser automation itself.
