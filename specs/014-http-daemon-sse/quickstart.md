...
2. Run `uv run ruff check spanweave/daemon tests/test_daemon.py`.

## Manual spot checks

1. Run `uv run python -m spanweave.daemon --repo . --host 127.0.0.1 --port 8765` and confirm the listener binds to loopback.
...
6. Retry any route without `X-Spanweave-Secret` or with a non-loopback client address and confirm the daemon rejects the request.
